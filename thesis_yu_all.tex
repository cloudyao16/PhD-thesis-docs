...
atmosphere. As an important aging process, cloud processing can change
particle size and composition through cloud chemistry and in-cloud
coagulation. Cloud processes are highly affected by per-particle
properties, through determining which particles can be activated and
which reactions occur for each particle. It
is challenging for global or regional models with simplified aerosol
representations to accurately capture these processes.
...
aerosol. By coupling an aqueous chemistry mechanism to
...
bins or modes. Aqueous-phase chemistry processes cause aerosol
populations to be more internally mixed, and CCN concentrations
increase substantially after cloud processing for supersaturation
levels lower than the maximum cloud supersaturation. I also found
coagulation within clouds had a negligible impact on aerosol mixing
state.

The aim of the second part of the thesis was to systematically quantify
the impact of aerosol mixing state on aerosol optical properties and
the role of RH. I created a reference scenario library with aerosol
populations of a wide range of mixing states using the
...
reaching up to 70\% for externally-mixed populations. The error
was smaller for particle scattering, with a maximum of $-$32\%
scattering coefficients underestimation. I also found case with larger underestimation in single
scattering albedo ($-$22\%) was associated with the higher scattering underestimation. The relative humidity
and associated aerosol water uptake only had a small impact on the
...
I am lucky to join the Department of atmospheric science family in UIUC. I met lots of new friends and I will miss the spring picnic, fall picnic and Christmas party hold by DASSO. I would also like to thank my current and previous PartMC group members. Special thanks to Jeff Curtis for his coding helps and Joseph Ching for his thoughtful ideas. 

I am also thankful to my friends for providing the friendship and I will miss the joyful times we spent together in different trips. I also want to express my special thanks to Shiya Liu, Beixi Jia, Shan Huang, Qing Zhang for lots of interesting and meaningful discussions about social issues.

Lastly, I especially thank my mom, dad and brother. Thanks for their understanding and unconditional love they provided, especially during the unusual past two years.
...
\citep{anenberg2010estimate}. According to a recent WHO report \citep{world2018air}, 
ambient air pollution caused around 4.2 million premature deaths, 
including 300,000 children under age of 5. The majority of these children died as a result of aerosol particle exposure, 
mainly in low- and middle-income nations.
...
(secondary aerosol). The major components of particles are sulfate, nitrate
, carbonaceous species, sea salt and mineral dust. These species can originate from
...
ocean regions through wave breaking and bubble bursting. Dust is another primary particle type originating mainly
...
\citep{shao2019heterogeneous, zheng2020multiphase}. Nitrate is mainly formed 
through oxidation of $\rm NO_2$ by OH at daytime and hydrolysis of $\rm N_2O_5$ at 
night \citep{alexander2009quantifying}. As for ammonium particulate, 
it forms when ammonia partitions over the surface of an acidic particle \citep{zhuang1999size}. 
Black carbon (BC) and primary organic aerosol (POA)
...
and aqueous chemistry \citep{zhu2017mechanism, lim2010aqueous, griffin2013sources,
  mcneill2015aqueous}. Oligomerization is a process that combines monomers into macromolecular compounds, 
  and the oligomerization of primary monomers and its decomposition products are proved to be an important pathway for SOA
  formation \citep{tolocka2004formation}. 
...
As an example, Figure~\ref{fig:chap1-mixing} shows
transmission electron microscopy (TEM) images of particles collected
in urban Shanghai in 2010. Some particles are mixture of two
...
condensation. These particles can stay longer in the air due to less 
efficient coagulation and wet removal processes, and the lifetime of these particles
can be up to one week \citep{feichter2009climate}. As for coarse-mode particles, it is hard to grow
such large particles directly from accumulation mode
\citep{friedlander1991scavenging, lee2005size}. They are mostly originated
from natural sources and produced through mechanical techniques. Rapid gravitational
settling results in short lifetime for these particles.
...
effects. However, there still exists large uncertainties in quantifying the second effects,
and it is not included in calculating aerosol-cloud interaction forcings in Fig.~\ref{fig:chap1-aerosol-climate}.
...
analyzing satellite observations in four regions over Atlantic ocean where 
clouds are affected by four different types of aerosols, proved that polluted
clouds are with more smaller droplets. Field observations also confirm that more cloud condensation
nuclei (CCN) generate more cloud droplets with smaller sizes in liquid
clouds \citep{jia2019distinct,kleinman2012aerosol}. 
...
droplets but also inhibits the precipitation development. This
...
to the complex microphysical and macrophysical buffer responses after altering
aerosol loading in the environment \citep{stevens2009untangling}.
...
concentrations by 7--17\%. Using the CCN sampled from 2010 CalNex field campaign,
\citet{moore2012hygroscopicity} also found internal mixture resulted
in 30--75\% overprediction of CCN concentration. \citet{ervens2010ccn}
conducted a more comprehensive closure study by using aerosol data
collected from six different locations. They found, for regions that are affected by
fresh emission, detail size-resolved aerosol information are required to make a reasonable CCN
 prediction, while for regions are with aged aerosols, CCN can be well-predicted with simple
...
particle. The absorption enhancement
of BC-containing particles due to its coatings have been widely
investigated \citep{Moffet2009,Liu2017, wu2020light}. The distribution
of the non-absorbing species over the population were found to be the
...
enhancement. It can be  more complicated if considering BC shapes. By using discrete dipole approximation
...
populations, and motivates the work presented in Chapter~\ref{chap2} and 
Chapter~\ref{chap3} of this thesis. Figure~\ref{fig:chap1-aq-proc} shows the chemical and
...
campaign over west Chilean coast \citep{kleinman2012aerosol} and the MASE campaign at the central
California coast \citep{hudson2015cloud}. Besides
...
Chapter~\ref{chap4} evaluates the error in aerosol absorption and scattering,
...
\label{chap2}
This chapter presented the application of PartMC as a particle-resolved
cloud parcel model that includes aqueous-phase chemistry. I first
described the PartMC model framework, and the coupled cloud parcel and
aqueous chemistry module as it was used in both this chapter and
Chapter~\ref{chap3}. Then, I presented a comparison to other size-resolved and bulk
cloud chemistry models. Furthermore, I showed results from an ensemble
...
Clouds cover around 70$\%$ of the earth \citep{stubenrauch2013assessment} and create an
...
environment \citep{Zaveri2010a,fierce2015explaining}. It has also been used as a cloud parcel model to investigate the impact of
aerosol mixing state on cloud droplet formation \citep{ching2012impacts, Ching2016}. However, the model did not have
...
Table~\ref{tab:capram}. 

When coupling CAPRAM with PartMC, the original gas phase chemistry mechanism, regional atmospheric chemistry modeling
(RACM), used in CAPRAM 2.4 was replaced with CBM-Z in PartMC-MOSAIC. It is worth 
noting that MOSAIC is not running when we have aqueous chemistry. In the current setting,
aqueous chemistry, and the evaporation and
...
were combined with their first acid dissociation step. The coupling work 
was accomplished by Matt Dawson and the codes was available online (https://github.com/compdyn/partmc/tree/aqchem).
...
updraft velocity, we obtained the temperature and pressure values of constant
updraft velocity at 0.5 m/s from the pyrcel model, a zero-dimensional
adiabatic cloud parcel model \citep{rothenberg2016metamodeling}, with the timestep
of every 60s.
...
differences. Actually, it turned out this assumption is not making a difference. 
...
table~\ref{TMI-setting}. Gas $\rm SO_2$ was selected because it provides the sources of S(IV). 
The gas species $\rm O_3$ and $\rm H_2O_2$ were selected because they are the important oxidants for sulfate formation.
Ammonia also plays an important role in aqueous sulfate formation by changing environment pH.
The range of mixing ratios for the different gas phase
...
\citep{peng2005importance}. It is worth mentioned that updraft velocities can change 
during the simulation period due to droplets formation. A total of $3^5$ = 243 cases were run using all possible
...
be 0.05 and 0.01. We selected these two Fe mass fraction values to produce 
Fe molar concentration consistent with previous studies \citep{Deguillaume2005}. 
Otherwise the setup was the same as for $P_{\rm ref}$, which
...
\citep{deguillaume2004role}. Since OH was quickly depleted, additional
...
for all scenarios, stratified by the five different input parameters, including
$\rm NH_3$, $\rm H_2O_2$, $\rm O_3$, $\rm SO_2$, and temperature lapse rate. 
...
separation is not as clear for the other four parameters. Cases with
higher sulfate formation are seen with all the four levels of
temperature lapse rate, $\rm H_2O_2(g)$ and $\rm O_3(g)$ mixing
ratio. But we can also found, the highest sulfate producing rates are associated
with high $\rm O_3$ and $\rm SO_2$ conditions. This phenomena indicates that the ammonia gas phase mixing
ratio is a determining factor for aqueous sulfate formation in the
simulated scenarios under consideration. Next, we will look into the underlying mechanism. It is worth mention that we currently analyzed the sulfate production rates by connecting with
the initial gas mixing ratios. However, aqueous sulfate formation 
rates depend on what is actually in the aqueous phase and it changes with time. 
...
\begin{figure}[ht]
    ...
    \caption{Sulfate time series categorized by five different
      ...
      mixing ratio, (d) initial $\rm O_3(g)$ mixing ratio, and (e) initial $\rm SO_2(g)$ mixing ratio. The
      three colors and symbols are for the low, medium and high levels
      of each parameter.}
    \label{chap2:ens_three}
\end{figure}

Gas $\rm SO_2$ is highly soluble to water and it dissolves rapidly. The dissolved
...
pathways and transition pathway reaction~\ref{eq:chem-eq} will be discussed in next section.
...
pH leads to high S(IV) solubility and reaction rates increased (the cases in the
red boxes of Fig~\ref{chap2:reac-rates}). The
...
\begin{figure}[ht]
    ...
    \caption{Relationship between aqueous sulfate formation rates and
      ...
      0.5$~\unit{ppb}. Red boxes are for the aerosol
      populations with only differences in $\rm NH_3(g)$ and are
      analyzed for detail in the text.}
...
\section{Conclusion}
\label{chap2.6}
For the work presented in this chapter, we used particle-resolved model
...
%%%Future work%%%
Currently, We used monodisperse aerosol populations for analysis. In
the future, we can generalize our findings by more realistic
...
OH(g) concentrations. On the one hand, Fe from different sources have different
...
reactions. On the other hand, the need for OH emissions is an artefact of not having entrainmentor 
gas-phase chemistry happening at the same time. If we had entrainment, 
then fresh OH would be supplied from outside the
cloud and if we had gas-phase chemistry going on, OH would
be produced. However, the way we did it right now is that we start
with a given gas initial condition and that gets used up and only aqueous chemistry is going on. 
Current work can be extended by considering entrainment or gas-phase chemistry happening with cloud chemistry at the same time .
...
This chapter quantified the changes of aerosol properties after cloud processing using a particle-resolved aqueous chemistry model. Model details, mixing state metrics and cloud parcel simulation design were introduced first. Then, I analyzed the changes in number and mass distribution changes after cloud processing. At the end, I quantified the changes in aerosol mixing state, CCN concentration and aerosol optical properties after cloud processing. The first part about cloud processing effects on aerosol mixing state was in revision for \textit{Journal of Geophysical
  Research-Atmosphere} and the second part about the changes in aerosol optical values was in preparation for \textit{Aerosol Science and
  Technology}. 
...
number concentration \edits{and aerosol optical properties?} (3) What is the role of coagulation between the
interstitial particles and cloud droplets for mixing state of the
aerosol?
...
The corresponding figure for the low-emission case is shown as
  Figure~\ref{fig:sup1} in the Supporting Information. As expected, for this case,
  ...
  mixing ratio was only 1.2~ppb. The mixing state metrics (Figure~\ref{fig:sup1}d)
...
The CDNC is comparatively high for the example shown in
  ...
  Figure~\ref{fig:max_ss-cycle} (but always larger than 1000~$\rm cm^{-3}$). For the
...
The size distributions for the low-emission case are shown in
  Figure~\ref{fig:sup2}. The average mode diameter of the cloud droplet mode after
...
It is worth mentioning that our conclusion that CCN
  ...
  particles (Figure~\ref{fig:max_ss-cycle}a), and the cloud droplet concentrations
  accordingly decrease in the following cycles (Figure~\ref{fig:max_ss-cycle}b). This is
...
\edits{\subsection{Changes in aerosol scattering and absorption abilities} 
\label{sec:aq-opt}
The changes in aerosol size distribution and components affect aerosol optical properties. 
We analyzed the aerosol optical ability changes in high-emission and low-emission cases after first cloud cycle. As shown in 
Figure~\ref{fig:cloud-opt}, all populations are experiencing enhanced scattering ability after cloud processing, in consistent with the findings
by \citet{yuskiewicz1999effects} and \citet{romakkaniemi2006influence}. The enhancement was more obvious for low-emission case, where median scattering coefficient increased from $1.3\times10^{-4}$ to 10.9$\times 10^{-4}$~\unit{m^{-1}}. 

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{chap3_figs/fig14.pdf}
    \caption{Optical value changes after cloud processing. (a) is for scattering coefficient $\beta_{\rm scat}$. (b) is for absorption coefficient $\beta_{abs}$. Each point is for a urban plume case and color is for the plume hour of the case. Circle is for high-emission case and triangle is for low-emission case.}
    \label{fig:cloud-opt}
\end{figure}

As for absorption coefficients, there is no significant changes after cloud processing, and this can be explained by the less BC-containing particles were activated. As shown in Figure~\ref{fig:bc-2d}, in both high-emission and low-emission case, the activated particles are all with less than 10\% of BC.Those with with BC fraction are smaller particles and they did not grow during the cloud processing. 
 ...
 \begin{figure}[H]
    ...
    \caption{Two-dimensional number concentration distribution
      ...
      after cloud processing (c,d) for aerosol populations from $t_{\rm u}$=12~\unit{h}. 
      The left column is for high-emission case and the right column is for low-emission case.}
    \label{fig:bc-2d}
\end{figure}
}
%
...
\edits{We also found cloud processing enhance particle scattering abilities due to particle growth, and 
the effects are more profound for high-emission case, with median scattering coefficient increasing from $1.3\times10^{-4}$ to 10.9$\times 10^{-4}$~\unit{m^{-1}}. The effects are minimal in changing aerosol absorption abilities, and less activated BC-containing particles can be the reason.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%Chapter 4%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Quantifying the effects of mixing state on aerosol optical properties}
This chapter developed a framework to systematically quantify the errors introduced by simplified aerosol representation. 
I first introduced the processes to create reference and sensitivity scenarios. Then, I analyzed the errors in aerosol absorbing and scattering optical abilities introduced by internal mixture assumptions used by size-bin aerosol models. The work is in
preparation for \textit{Atmospheric Chemistry and Physics}.
...
Approximation model, \citet{scarnato2013effects} found that for
...
and Modal Aerosol Module (MAM)\citep{Binkowski2007,
  Appel2017,Liu2012}.  The modes are externally mixed from each other,
...
concentrations were evaluated at, and aerosol mixing state. In this
...
fractions because of the maximum $\Delta_{\rm core}$ is reached. As
the table (Fig.\ref{fig5:abs-exp})shows, for a 300~\unit{nm}
population, the normalized absorption is 0.76 when the particles are
internally mixed, higher than an external mixed population
(0.5). Although this example is an idealized case since our
populations lie between external and internal mixtures before
composition-averaging and are polydisperse, this illustrates that
assuming internal mixture will lead to absorption overestimation.
...
To further explore the effects of coating
volume changes, Fig.~\ref{fig6:scat-err}(b) shows the size-resolved
scattering coefficients before and after composition-averaging for
aerosol populations from scenario 77 at 2h, which produced the largest
scattering coefficients underestimation ($-$32\%). We found there is
significant decrease of $\sigma_{\rm scat}$ in size range of
...
The blue lines in Fig.~\ref{fig8:scat-exp} show the scattering
cross sections for two different real refractive indices. For
particles with diameters between 800 and 1200~\unit{nm}, a lower
refractive index leads to a larger scattering cross section, even though the different is smaller
than the change caused by adding a BC core. Similar to the BC core
size change $\Delta D^{\rm core}$ {in Figure~\ref{fig3:abs-error}},
we defined a volume-weighted refractive index change, $\Delta{m^{\rm
    real}}$, to help understand the changes in scattering. The index
...
catelyzed by TMI can be important, but I have not systematically
