...
which reactions occur for each particle depending on its acidity. It
...
The aim of the first part of this thesis is to (1) quantify the
...
coagulation within clouds has a negligible impact on aerosol mixing
state.

The aim of the second part of the thesis is to systematically quantify
...
reaching up to 70\% for more externally-mixed populations. The error
was smaller for particle scattering, with a maximum of $-$32\%
scattering coefficients underestimation. I also found cases with larger underestimation in single
...
through oxidation of $\rm NO_2$ by OH at daytime and hydrolysis of $\rm N_2O5$ at 
...
\citet{Hoppel1986} evident in particle populations processed by
...
\begin{figure}
	...
	\caption{Aerosol numerical approaches:(a) Bulk model, (b) Modal model, (c)
          Section model, and (d) Particle-resolved model.}
...
\textcolor{red}{Change ``paper'' and ``section'' to ``Chapter''} The
paper is structured as follows: Model description, scenario design and
the definition of metrics are given in
Sect.~\ref{sec:methods}. Section \ref{sec:dry-case} shows the relation
between the errors in aerosol scattering and absorption and mixing
state for dry aerosol populations, and section \ref{sec:wet-case}
...
analyzed in Sect. \ref{sec:ssa}. Section \ref{sec:conclusion}
