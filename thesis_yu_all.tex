...
was smaller for particle scattering, with a median of $-$1.2\%
scattering coefficients underestimation. The relative humidity
(associated and aerosol water uptake) only has a small impact on the
magnitude of these errors.

\textcolor{red}{given that you quote 70\% error for absorption (which
  I think is good), I would not quote the median error for
  scattering. I also would quote ``up to xx'' for scattering. I would
  also add something about SSA. And the minus sign is just $-$, not
  $--$!}
\end{abstract}
...
Aerosol size distribution evolves after the formation of aqueous inorgaincs and organics. Hoppel minima, which describes the CCN concentration gap between two particle distribution modes peak at 0.02--0.03 and 0.08-0.15~$\rm \mu m$ respectively,  was found by \citet{Hoppel1986} when analyzing the particles processed by nonprecipitation clouds. This phenomena has further been observed for the particles at different region, such as VOCALS campaign over west Chilean coast at Arica and MASE experiment over central California coast \citep{kleinman2012aerosol, hudson2015cloud}. Besides the aqueous aerosol formation, particle size distribution also changes due to coagulation between interstitial particles and cloud droplets. \citet{pierce2015importance} found this process can reduce total particle number concentration by 10--15\% globally. 
...
\textcolor{red}{Add intro -- this Chapter was submitted to ....}
...
\textcolor{red}{Add intro -- This chapter is prepared for submission to ....}
...
\textcolor{red}{Change ``paper'' to ``chapter''.}
...
absorption aerosol optical depth. \textcolor{red}{Does neglecting
  mixing state mean assuming internal mixture?}
...
properties was quantified by
\citet{nandy2021water}. \textcolor{red}{If you say this, you need to
  say briefly what they found!}
...
\textcolor{red}{I think we should always write
  ``composition-averaging'' (with a dash.}
...
\textcolor{red}{I think the red rectangles can be removed.}
...
and West (2013) \textcolor{red}{citation hardcoded!}. These metrics
...
\begin{figure}
	...
	\caption{Normalized absorption coefficient as a function of BC
          ...
          and yellow for coating species. \textcolor{red}{The figure
            needs to be rearranged so that it doesn't extend into the
            margin (Tammy won't accept it)}}
	\label{fig5:abs-exp}
\end{figure}
...
The current analysis was based on particles of the same sizes. Hoever,
...
refractive index of the coating. As Fig.~\ref{fig8:scat-exp}(a) shows,
...
\textcolor{red}{in Figure 4.6.b, this is the overall result, we cannot
  say that the decrease comes from changes in BC core or changes in
  coating ref. index, correct? I'm thinking that 4.6.b should be next
  to Fig. 4.5 because it highlights one particular point.}

To further explore the effects of coating
volume shrink, Fig.~\ref{fig8:scat-exp}(b) shows the size-resolved
...
\textcolor{red}{I can't seem to find a point at -32\% in Fig. 4.5?}
...
\begin{figure}
	...
	\caption{(a) Relation between scattering ability, refractive
          ...
          core of $0.2D$. (b) Size-resolved scattering coefficients at
          reference and sensitivity scenario library.  This population
          is from scenario 77 at 2h.}
	\label{fig8:scat-exp}
\end{figure}

The blue lines in Fig.~\ref{fig8:scat-exp}(a) show the scattering
...
refractive index leads to a larger scattering cross section
(Fig.~\ref{fig8:scat-exp}(a)), even though the different is smaller
than the change caused by adding a BC core. Similar to the BC core
size change $\Delta D^{\rm core}$ \textcolor{red}{in Figure~\ref{xx}},
...
\end{equation}
\textcolor{red}{Is V-i the volume of the coating? Should be, right?}
...
\textcolor{red}{I don't understand the following paragraph. I think
  the only thing that we need to say is that the decrease in relative
  error for scattering for the high relative humidities is due to the
  considerable increase in the magnitude of the absolute volume
...
\textcolor{red}{It occurs to me that the spread in scattering error
  could also be explained by core size change. did you try coloring
  figure 4.5 by core size change (same as figure 4.3)?}
...
This dissertation describes the work to improve the understanding of aerosol aging processes in clouds and quantify the role of aerosol mixing state in determining its scattering and absorbing abilities. The key questions this work is going to answer are: (1) To what extent does cloud
processing change the aerosol mixing state of the population that entered the cloud, and what is the role of coagulation between the
interstitial particles and cloud droplets for mixing state of the aerosol? (2) Given the degree of the aerosol population mixing state, 
what are the errors in aerosol optical values introduced by internal mixture assumption and what are the causes? 

In answering the first question, I designed cloud simulations by using the newly developed particle-resolved aqueous chemistry model. It is a zero-dimensional adiabatic cloud parcel model with comprehensive aqueous reactions. The model was evaluated by comparing with other state-of-art aerosol models and was proved to be capable of capturing the complex aqueous sulfate reactions (Chapter~\ref{chap2}). I quantified aerosol mixing state changes after aqueous chemistry processes and generalized the findings by using aerosol populations from different urban environment. I also evaluated the role of coagulation between interstitial particles and cloud droplets for aerosol mixing state (Chapter~\ref{chap3}). 

For the second question, I developed a framework to quantify the error in aerosol optical values introduced by simplified aerosol representation (Chapter~\ref{chap4}). I created an an reference aerosol population ensemble with a wide variety of aerosol mixing state by running particle-resolved models. I quantified the errors by creating another sensitivity scenario with aerosol populations of internal mixture assumptions in prescribed size bins, which was accomplished by employing composition-averaging strategy. I developed several metrics to explain the errors. I further explored the impact of water update on the optical value errors by exposing these populations at different RH levels.

\section{Contributions}
{\bf Quantifying cloud processing effects on aerosol properties.} With the newly coupled particle-resolved aqueous chemistry model, I quantified the 
changes of aerosol properties after cloud processing at per-particle scale. I found for aerosol populations dominated by fresh emissions, adding secondary species produced from aqueous chemistry increased the particle diversity. But whether the overall population became more or less internally mixed depends on the relative changes average particle diversity ($D_{\alpha}$) and bulk aerosol diversity ($D_{\gamma}$), which determines aerosol mixing state metrics $\chi$. For most of the cases, aqueous chemistry processes resulted in a more interlly-mixed populations, as showed in Fig.~\ref{chap5-fig1}. More cloud cycles further made the populations more internally mixed, but a completely internal mixture was not achieved under current simulation cases. The change in mixing state due to cloud processing depended also on the fraction of particles that formed cloud droplets. In high-emission case, less than 23\% of particles are activated and the mixing state $\chi$ did not reach 90\%. For low-emission case, the activated particle portion can reach to 20\%--75\% and $\chi$ can be over 90\% after cloud processing. I also found, after cloud processing, CCN concentration increased substantially for supersaturation levels lower than the maximum cloud supersaturation. For an aged aerosol population, CCN concentration at supersaturation level 0.02\% increased from 30 to 547 ~\unit{cm^{-3}}. I also proved that coagulation within clouds had a negligible impact on aerosol mixing state. 
...
{\bf Systematically quantifying the error in aerosol optical properties.} I quantified the errors introduced by internal mixture assumption used in sectional aerosol models. I found size-resolved aerosol representation generally led to an overestimation of volume absorption coefficients and an underestimation of the volume scattering coefficients, and the errors were larger for externally mixed populations. For absorption, the relative error can reach to +70\% at dry environment and the magnitude was smaller for the scattering error. The amplitude was comparabel to what \citet{Ching2017} found for CCN error, as shown in Fig~\ref{chap5-fig2}. I identified that the increased BC core after composition-averaging was responsible for absorption errors, and it was the coating volume shrink resulting in the scattering coefficients underestimation. I showed RH had little effects in changing the optical value errors brought by internal mixture assumption. 

\begin{figure}[H]
    %\centering
    \includegraphics[scale=0.35]{chap5_figs/chap5-fig2.pdf}
    \caption{Relative error in (a) CCN concentration at supersaturation levels between 0.05\% to 1\%, (b) absorption coefficients and (c) scattering coefficients. (a) is adapted from \citet{Ching2017}. Check Fig.~\ref{fig3:abs-error} and Fig.~\ref{fig6:scat-err} in Chapter~\ref{chap4} for figure (b) and (c) details}
    \label{chap5-fig2}
\end{figure}

\section{Future work suggestions}
{\bf Sensitivity of aqueous sulfate formation to per-particle properties.} I proved the capability of the particle-resolved aqueous chemistry approach 
to simulate the complex aqueous sulfate reaction systems. Since aqueous sulfate formation is very sensitive to particle acidity, particles with different acidity will experience different sulfate growth rates. So, the work should be further extended to quantify the errors if per-particle properties are not considered. Also, I currently figured out the sulfate formation catelyzed by TMI can be important. But I did not know at which environment this pathway can be efficient. More simulations with varied conditions can be conducted to generalized the cases with efficient TMI-catalyzed sulfate formation. 

{\bf Considering the absorption of organic carbons.} In current work, I did
...
in quantifying aerosol absorbing abilities. In the fu

{\bf More accurate representations for BC-contained particles.} Currently, I used refractive index value 1.82 + 0.74$i$ for BC, which is
close to the medium index suggested by \citet{stier2007aerosol}, but it can vary among different measurements.  Another important factor for ca I used Mie theory with core-shell configuration 
to calculate optical properties assuming spherical particle shapes. Our results are therefore most representative of
BC-containing populations, where the BC core is collapsed rather than
...
more irregular particle shapes \citep{scarnato2013effects,curtis2008laboratory,luo2019optical, wu2020light}

{\bf The importance of mixing state representation for dust and sea salt aerosols in determining its radiative abilities.}In current aerosol populations, I focused on fine mode particles over urban regions. Sea salt and dust particles were not included when creating the scenario library populations. Considering the large coverage of ocean and desert, it should also be important to include these particles for reducing the uncertainties in direct radiative forcing.
