...
after composition-averaging, the change of the BC core size and
corresponding change in coating thicknesses, and the change in the
...
The change of refractive index is the other factor that affects
particle scattering. For particles with diameter between 800 and
1200~\unit{nm}, lower refractive index leads to higher scattering
ability (Fig.~\ref{fig8:scat-exp}(a)), even though the amplitude is
smaller than the change brought by adding a BC core. Similar to the BC
core size change $\Delta D^{\rm core}$, we defined a volume-weighted
refractive index change, $\Delta{m^{\rm real}}$, to help understand
the changes in particle scattering ability. The index change is
defined as:
\begin{equation}
